  ...
  return [
    ...
    node('eng-dashboard', 4300, -100, 'Дашборд и календарь', 'screen', 'engagement',
      phGroup([['calendar_day_viewed', 'Открыл день в календаре'], ['date_range_selected', 'Выбрал диапазон дат'],
        ...
        ['dashboard_widgets_managed', 'Настроил набор виджетов'], ['dashboard_widget_settings_opened', 'Открыл настройки виджета']]).subtitle,
      phGroup([['calendar_day_viewed', 'Открыл день в календаре'], ['date_range_selected', 'Выбрал диапазон дат'],
        ['data_refreshed', 'Обновил данные вручную'], ['calendar_month_navigated', 'Пролистал месяц'],
        ['dashboard_widget_view_toggled', 'Переключил вид виджета'], ['dashboard_layout_toggled', 'Изменил layout дашборда'],
        ['dashboard_widgets_managed', 'Настроил набор виджетов'], ['dashboard_widget_settings_opened', 'Открыл настройки виджета']]).evidence, 'PostHog · 8 событий (calendar_*, dashboard_*, date_range_selected, data_refreshed)'),
    ...
    node('eng-trade-detail', 4300, 220, 'Детали сделок', 'screen', 'engagement',
      phGroup([['trade_detail_block_added', 'Добавил блок в детали сделки'], ['manual_trade_creation_started', 'Начал создание сделки'],
        ['trade_template_selected', 'Выбрал шаблон сделки'], ['trade_detail_column_added', 'Добавил колонку в детали'],
        ['trade_template_saved', 'Сохранил шаблон сделки'], ['item_favorited', 'Добавил в избранное']]).subtitle,
      phGroup([['trade_detail_block_added', 'Добавил блок в детали сделки'], ['manual_trade_creation_started', 'Начал создание сделки'],
        ['trade_template_selected', 'Выбрал шаблон сделки'], ['trade_detail_column_added', 'Добавил колонку в детали'],
        ['trade_template_saved', 'Сохранил шаблон сделки'], ['item_favorited', 'Добавил в избранное']]).evidence, 'PostHog · 6 событий (trade_detail_*, trade_template_*, manual_trade_creation_started, item_favorited)'),
    ...
    node('eng-growth', 4300, 700, 'Рост и шеринг', 'screen', 'engagement',
      phGroup([['content_shared', 'Поделился контентом'], ['app_store_link_clicked', 'Клик по ссылке на app store'],
        ['referral_link_copied', 'Скопировал реферальную ссылку']]).subtitle,
      phGroup([['content_shared', 'Поделился контентом'], ['app_store_link_clicked', 'Клик по ссылке на app store'],
        ['referral_link_copied', 'Скопировал реферальную ссылку']]).evidence,
      'PostHog · 3 события (content_shared, app_store_link_clicked, referral_link_copied)'),
...
function buildEdges(): Edge[] {
  return [
    ...
    edge('e-resumed-choose', 'onboarding-resumed', 'choose-path', undefined, ((posthogCounts.value['onboarding_resumed'] ?? 0) >= (posthogCounts.value['onboarding_path_selected'] ?? 0) * 0.5)),
